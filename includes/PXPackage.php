...
abstract class PXPackage {
	...
	public static function getPackageField( $fieldName, $fileData, $packageData, $escapeHTML = true, $isWikitext = false ) {
		...
		if ( $isWikitext ) {
			...
			$parserOutput = $parser->parse( $value, $packagesTitle, ParserOptions::newFromAnon(), false );
			return $parserOutput->runOutputPipeline( $parser->getOptions() )->getContentHolderText();
		}
		if ( !$escapeHTML ) {
			return $value;
		}
